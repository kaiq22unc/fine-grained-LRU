...
//test just below highwatermark
void self_tests1() {
    int rv;
    for (int i = 0; i < HIGH_WATER_MARK - 1; i++) {
        for (int j = 0; j < 64; j++) {
            rv = reference(i);
            if (!rv) die("Failed to reference key %d\n", i);
        }
    ...
    sleep(3);
    print();
    rv = reference(HIGH_WATER_MARK - 1);
    //print();
    //rv = reference(HIGH_WATER_MARK);
    if (!rv) die("Failed to reference key %d\n", HIGH_WATER_MARK);
    print();
}

//reach  high watermark
void* self_tests2(void* arg){
    int rv;
    ...
    sleep(5); //wait for clean
    print(); // should print empty since clean() should be done
    rv = reference(HIGH_WATER_MARK);
    print(); // should print key high water mark, and refcount 1
    return NULL;
}

//test low watermark, should hang
void self_tests3(){
    int rv;
     
    for (int i = 0; i < LOW_WATER_MARK; i++) {
        rv = reference(i);
        if (!rv) die ("Failed to reference key %d\n", i);
    }
...
void self_tests4(){
    pthread_t t1;
    pthread_create(&t1, NULL, self_tests2, NULL);
    sleep(3); //wait for self_tests2 to reach high water mark then clean
    clean(0);
    pthread_join(t1, NULL);
    clean(0);
    print(); //should print empty list
    printf("end self_tests4\n");
}
